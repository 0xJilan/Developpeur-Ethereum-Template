...
contract Spa {
    ...
    function Get(uint256 _id) public view returns (Animal memory fifou) {
        fifou = Animaux[_id];
    }
    ...
    function GetAdoption(address _addr)
        public
        view
        returns (Animal memory Rex)
    {
        Rex = Animaux[Adoption[_addr]];
    }
    ...
    function AdoptIfMax(
        ...
        for (uint256 i; i < Animaux.length; i++) {
            if (
                ...
                keccak256(abi.encodePacked(Animaux[i].race))
            ) {
